...
(function() {
	...
	on('chat:message', (msg) => {
        ...
        if (state[state_name][caller.id].config.overwrite) {
            let objects = findObjs({
                _type: 'character',
                name: state[state_name][caller.id].config.prefix + character.name + state[state_name][caller.id].config.suffix
		...
		if (object === null) {
            ...
            object = createObj('character', {
                name: state[state_name][caller.id].config.prefix + character.name + state[state_name][caller.id].config.suffix,
                inplayerjournals: playerIsGM(msg.playerid) ? state[state_name][caller.id].config.inplayerjournals : msg.playerid,
                controlledby: playerIsGM(msg.playerid) ? state[state_name][caller.id].config.controlledby : msg.playerid
            });
        }
		...
		addAdvantagesOrDisadvantages(character.disadvantages, repeating_attributes, true);
		...
		// Weaksauce
		setTimeout(() => {
			sendChat(script_name, `<div style="${style}">Import of <b>${character.name}</b> has completed.</div>`, null, {noarchive:true});
		}, 5000);
	});
	...
	const addPoints = (character, single_attributes) => {		
		Object.assign(single_attributes, {
			...
			'totalpoints': Object.values(character.creationPoints).reduce((a, b) => a + (b || 0), 0)
		});
	}
	...
	const addSpecializations = (character, repeating_attributes) => {
		...
		for (const specialization of character.skillSpecializations) {
			...
			if (i === 0) {
				...
				attributes['specializationpips'] = dice.length === 2 ? dice[1].substring(1) : 0;
			} else {
				row = getRepeatingRowIds('specialization', 'specializationname', specialization.name, 0);
				
				attributes[`repeating_addspecialization_${row}_addspecializationname`] = specialization.name;
				attributes[`repeating_addspecialization_${row}_addrootskill`] = specialization.baseSkill;
				attributes[`repeating_addspecialization_${row}_addspecializationdice`] = dice[0];
				attributes[`repeating_addspecialization_${row}_addspecializationpips`] = dice.length === 2 ? dice[1].substring(1) : 0;				
			}
	...
	const addAdvantagesOrDisadvantages = (list, repeating_attributes, isDisadvantage = false) => {
		...
		for (const item of list) {
			...
			if (i === 0) {
				...
				attributes[`${prefix}advantagecost`] = Array.isArray(item.cost) ? item.cost[0] : item.cost;
			} else {
				row = getRepeatingRowIds(`${prefix}advantages`, `${prefix}advantagename`, item.name, 0);
				
				attributes[`repeating_add${prefix}advantages_` + row + `_add${prefix}advantagename`] = item.name;
				attributes[`repeating_add${prefix}advantages_` + row + `_add${prefix}advantagedetails`] = item.details;
				attributes[`repeating_add${prefix}advantages_` + row + `_add${prefix}advantagecost`] = Array.isArray(item.cost) ? item.cost[0] : item.cost;						
			}
	...
	const addPowers = (character, repeating_attributes) => {
		...
		for (const power of character.powers) {
			...
			if (i === 0) {
				...
				attributes[`repeating_addpowers_${row}_addpowerdetails`] = getPowerDetails(power);
				attributes[`repeating_addpowers_${row}_addpowerranks`] = ranks[0];
				attributes[`repeating_addpowers_${row}_addpowereffectiveranks`] = ranks[1];
	...
	const addMartialArts = (character, repeating_attributes) => {
		...
		for (const maneuver of character.martialArts) {
			...
			if (i === 0) {
				...
				attributes[`repeating_addmoves_${row}_addstrikebonus`] = maneuver.strike;
				attributes[`repeating_addmoves_${row}_addevasionbonus`] = maneuver.evasion;
				...
				attributes[`repeating_addmoves_${row}_addmovenotes`] = maneuver.notes || '';
			}
	...
	const sendConfigMenu = (player, first) => {
        ...
        let prefixButton = makeButton(prefix, '!gemini --config prefix|?{prefix}', buttonStyle);
        let suffix = (state[state_name][playerid].config.suffix !== '') ? state[state_name][playerid].config.suffix : '[NONE]';
        let suffixButton = makeButton(suffix, '!gemini --config suffix|?{suffix}', buttonStyle);
        let overwriteButton = makeButton(state[state_name][playerid].config.overwrite, `!gemini --config overwrite|${!state[state_name][playerid].config.overwrite}`, buttonStyle);
        let debugButton = makeButton(state[state_name][playerid].config.debug, `!gemini --config debug|${!state[state_name][playerid].config.debug}`, buttonStyle);
		...
		if (playerIsGM(playerid)) {
            ...
            inPlayerJournalsButton = makeButton(ipj, `!gemini --config inplayerjournals|?{Player|None,[NONE]|All Players,all${players}}`, buttonStyle);
            ...
            controlledByButton = makeButton(cb, `!gemini --config controlledby|?{Player|None,[NONE]|All Players,all${players}}`, buttonStyle);
        }
        ...
        let debug = '';
		let resetButton = makeButton('Reset', '!gemini --reset', `${buttonStyle} margin: auto; width: 90%; display: block; float: none;`);
    ...
    const sendHelpMenu = (player, first) => {
		...
		text += '<li>Open the exported file and copy the file contents to your clipboard</li>';
		text += '<li>Run the command `!gemini --import {}` making sure you replace `{}` with the contents of your clipboard</li>';
		text += '</ol>';
		text += '<hr>';
		text += `<b>Commands:</b> ${makeList(listItems, 'list-style: none; padding: 0; margin: 0;')}`;
        text += '</div>';
	...
	const getRepeatingRowIds = (section, attribute, matchValue, index) => {
        ...
        if (state[state_name][caller.id].config.overwrite) {
            let matches = findObjs({ type: 'attribute', characterid: object.id })
                .filter((attr) => {
                    return attr.get('name').indexOf('repeating_add' + section) !== -1 && attr.get('name').indexOf(attribute) !== -1 && attr.get('current') == matchValue;
                });
				
            for (const i in matches) {
				let row = matches[i].get('name').replace(`repeating_add${section}_`, '').replace(`_${attribute}`, '');
